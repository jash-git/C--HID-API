...
namespace HID_SIMPLE.HID
{
    ...
    public class Hid : object
    {
        ...
        int outputReportLength;//輸出報告長度,包刮一個字節的報告ID
        public int OutputReportLength { get { return outputReportLength; } }
        int inputReportLength;//輸入報告長度,包刮一個字節的報告ID   
        public int InputReportLength { get { return inputReportLength; } }

        /// <summary>
        /// 打開指定信息的設備
        /// </summary>
        /// <param name="vID">設備的vID</param>
        /// <param name="pID">設備的pID</param>
        /// <param name="serial">設備的serial</param>
        /// <returns></returns>
        ...
        private void ReadCompleted(IAsyncResult iResult)
        {
            ...
            {
                //MyConsole.WriteLine("設備無法連接，請重新插入設備");
        ...
        /// 獲取所有連接的hid的設備路徑
        /// </summary>
        /// <returns>包含每個設備路徑的字符串數組</returns>
        public static void GetHidDeviceList(ref List<string> deviceList)
        {
            ...
            if (hidInfoSet != IntPtr.Zero)
            {
                ...
                for (index = 0; index < MAX_USB_DEVICES; index++)
                {
                    ...
                    {
                        int buffsize = 0;
                        // 取得接口詳細信息:第一次讀取錯誤,但可以取得信息緩衝區的大小
